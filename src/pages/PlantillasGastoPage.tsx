...
export const PlantillasGastoPage = () => {
    ...
    const usarMutation = useUsarPlantilla();
    ...
    const [formData, setFormData] = useState<CreatePlantillaGastoDto>({
        nombre: '',
        categoriaId: 0,
        monto: null,
        descripcion: null,
    ...
    const gastoCategorias = useMemo(() => categorias.filter((c: any) => c.tipo === 'Gasto'), [categorias]);
    ...
    const handleUsar = async () => {
        ...
        try {
            await usarMutation.mutateAsync({
                ...
                data: { fecha: usarData.fecha, monto: usarData.monto },
            });
    ...
    const handleEdit = (p: PlantillaGasto) => {
        setEditingId(p.id);
        setFormData({
            nombre: p.nombre, categoriaId: p.categoriaId, monto: p.monto,
            descripcion: p.descripcion, tipo: p.tipo, cuentaId: p.cuentaId,
    ...
    const handleCloseModal = () => {
        ...
        setEditingId(null);
        setFormData({ nombre: '', categoriaId: 0, monto: null, descripcion: null, tipo: 'Variable', cuentaId: null, icono: null, color: null, ordenDisplay: 0 });
    };

    const openUsarModal = (p: PlantillaGasto) => {
        setSelectedPlantillaId(p.id);
        setUsarData({ fecha: new Date().toISOString().split('T')[0], monto: p.monto });
            ...
            <div className="max-w-7xl mx-auto px-4 py-8">
                ...
                {isLoading ? (
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-12 text-center">
                        ...
                        <p className="text-gray-600 dark:text-gray-400 mb-4">Crea plantillas para gastos que haces frecuentemente</p>
                        {!searchTerm && <button onClick={() => setIsModalOpen(true)} className="bg-emerald-600 text-white px-4 py-2 rounded-lg hover:bg-emerald-700"><Plus size={18} className="inline mr-1" /> Crear primera plantilla</button>}
                    </div>
                ) : (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                            ...
                            <div key={p.id} className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-5 border border-gray-200 dark:border-gray-700 hover:shadow-md transition-shadow">
                                <div className="flex justify-between items-start mb-3">
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{p.categoriaNombre} | {p.tipo}</p>
                                    </div>
                                ...
                                {p.monto && <p className="text-2xl font-bold text-gray-800 dark:text-white mb-1">${p.monto.toFixed(2)}</p>}
                                ...
                                <div className="flex justify-between items-center">
                                    ...
                                    <button onClick={() => openUsarModal(p)} className="bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 px-3 py-1.5 rounded-lg hover:bg-emerald-100 dark:hover:bg-emerald-900/50 flex items-center gap-1 text-sm font-medium">
                                        <Zap size={14} /> Usar
                                    </button>
                                </div>
                            </div>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4">
                                <div>
                                    ...
                                    <input type="text" value={formData.nombre} onChange={(e) => setFormData({ ...formData, nombre: e.target.value })}
                                        className="w-full px-3 py-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white" placeholder="Ej: Supermercado semanal" required maxLength={100} />
                                </div>
                                <div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <select value={formData.tipo || 'Variable'} onChange={(e) => setFormData({ ...formData, tipo: e.target.value })}
                                ...
                                <div>
                                    ...
                                    <input type="text" value={formData.descripcion || ''} onChange={(e) => setFormData({ ...formData, descripcion: e.target.value || null })}
                                ...
                                <div className="flex gap-2">
                                    <button type="submit" className="flex-1 bg-emerald-600 text-white py-2 rounded-lg hover:bg-emerald-700">Guardar</button>
                    ...
                    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                        <div className="bg-white dark:bg-gray-800 rounded-xl p-6 w-full max-w-sm">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex gap-2">
                                    <button onClick={handleUsar} className="flex-1 bg-emerald-600 text-white py-2 rounded-lg hover:bg-emerald-700">Crear Gasto</button>
